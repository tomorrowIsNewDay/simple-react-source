...
function renderVstateless(vstateless, parentContext) {
  ...
  if (!vnode || !vnode.vtype) {
    throw new Error(
      `@${factory.name}#render:You may have returned undefined, an array or some other invalid object`,
    );
  }
  return vnode;
}
...
export function renderComponent(component, parentContext) {
  ...
  if (!vnode || !vnode.vtype) {
    throw new Error(
      `@${component.constructor.name}#render:You may have returned undefined, an array or some other invalid object`,
    );
  }
  refs = null;
  return vnode;
